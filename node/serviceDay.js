...
module.exports.weekReport =
    function (queryDate, venueid, callback) {
        ...
        baseDate = baseDate + oneDay + (59*60*24*1000);
        ...
        for(var name in labelsArray) {
            datesArray[labelsArray[name]] = 10;
        };
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            collection.find({
                venueid: {$eq: venueid},
                captime: {$gt: daySeven, $lt: baseDate}
                ...
                if (err) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        capTime = new Date(result[i].captime);
